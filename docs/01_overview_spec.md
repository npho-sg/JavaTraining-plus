...
拡張シナリオ
:   
5a. ユーザー名またはパスワードが不正な場合エラーメッセージを表示する。
...
拡張シナリオ
:   
8a. Google認証に失敗した場合Google認証システムは認証失敗を表示する。
...
主シナリオ
:
    1. 管理者は引数でユーザー名、パスワード、Gメールを指定してGmail認証ユーザー登録機能を実行する。<br>
    2. サービス管理システムはトークンを発行する。<br>
    3. サービス管理システムは、トークンと登録者情報をGmail管理者登録テーブルにレコード追加する。<br>
    4. サービス管理システムは指定したGメールアドレスへ認証をURLを送信する。<br>
    5. 管理者は発行されたトークンをコンソールで目視する。<br>
    6. 管理者は受信した認証URLへアクセスする。<br>
    7. 管理者は、URLアクセス先で発行されたトークンを入力する。<br>
    8. サービス管理システムはトークン、有効期限、試行回数を検証する。<br>
    9. サービス管理システムは、ユーザー情報を登録する。<br>
    10. 管理者はユーザー情報を確認する。

拡張シナリオ
:   
    9a. トークンが不正な場合、サービス管理システムはエラーメッセージを表示する。<br>
    9b. トークンの有効期限が切れている場合、サービス管理システムはエラーメッセージを表示する。<br>
    9c. トークン3回間違えた場合、サービス管理システムはエラーメッセージを表示して、トークン再発行を行う。
