...
3. OAuth/OpenID Connectを利用した、認証後に取得したメールアドレスを基に利用者判定を行っている。

備考

・1.の手段を取る場合は、パスワードを平文のままにせず、ハッシュ化して保管すること。

・Gメール1つにつきアカウント1つまでしか登録できないこと。

・Google認証を利用して新規登録を行う場合は、登録されたGメールアドレス宛に認証トークンを送信し、メールアドレスの本人確認を行うこと。

・認証トークンには有効期限を設定すること。

・認証トークンの入力誤りが一定回数を超えた場合は、新しい認証トークンを再発行すること。

・認証完了後は認証トークンを無効化すること。

・認証トークンはコンソールにて目視で確認する。
